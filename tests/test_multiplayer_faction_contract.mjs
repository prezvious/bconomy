...
assert(server.includes('const revision = Math.max(0, Math.floor(Number(actor.profile.state_revision) || 0))'), 'Faction queries return migration-updated player revisions');
assert(server.includes('state: normalizePlayerState(actor.profile.state || {})'), 'Faction query revisions are paired with authoritative player state');
assert(api.includes("factionQuery('faction.joinMessage')"));
assert(api.includes("'X-Bconomy-API-Version': '2'"));
assert(api.includes('knownRevision: getRevision()'), 'Faction queries tell the server when the full player save is already current');
assert(api.includes('observedRevision > getRevision()'), 'Faction queries advance player revisions monotonically');
assert(api.includes('setState(data.state)'), 'Faction queries never advance a revision without its matching state');
