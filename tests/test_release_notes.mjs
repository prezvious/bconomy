...
// 1. Verify v4.2.0 exists and is marked latest
...
assert.strictEqual(latestRelease.version, 'v4.2.0');
assert.strictEqual(latestRelease.date, '2026-08-30');
assert.ok(latestRelease.title.includes('v4.2.0'));
assert.ok(latestRelease.sections.length >= 4, 'v4.2.0 should describe Activity views, pagination, accessibility, action performance, and documentation');
console.log('✓ Latest release v4.2.0 verified with title, date, and sections');

assert.strictEqual(packageJson.version, '4.2.0');
assert.ok(indexHtml.includes('Bconomy v4.2.0'));
assert.ok(indexHtml.includes('v4.2.0 Latest'));
assert.ok(releaseNotesMarkdown.includes('## [v4.2.0] — 2026-08-30'));
assert.ok(releaseNotesMarkdown.includes('Faster Authoritative Action Commands'));
assert.ok(releaseNotesMarkdown.includes('Rewards, cooldowns, shared faction effects'), 'Performance notes clearly state unchanged game behavior');
...
const expectedVersions = ['v4.2.0', 'v4.1.1', 'v4.1.0', 'v4.0.1', 'v4.0.0', 'v3.2.0', 'v3.1.1', 'v3.1.0', 'v3.0.0', 'v2.2.2', 'v2.2.1', 'v2.2', 'v2.1', 'v2.0', 'v1.4', 'v1.3', 'v1.2', 'v1.1', 'v1.0'];
const actualVersions = RELEASES.map(r => r.version);
assert.deepStrictEqual(actualVersions, expectedVersions, 'Releases must be ordered chronologically descending (v4.2.0 -> v1.0)');
